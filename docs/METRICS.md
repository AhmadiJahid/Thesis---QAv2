...
- `max_nodes_for_optimizer` (16): above it the optimizer is not called and the value is the
  ...
  **the only guard that actually bounds the cost**, because it is checked before any search
  ...
  timing (the gold's size is part of the cost, and so is how closely the predicted step labels
  tie with the gold's — not the edge count): the worst case measured at the cap is ~1.0 s
