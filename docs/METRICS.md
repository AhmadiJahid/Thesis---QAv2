...
| `ged_fallback_counts` | how many items did not get the optimizer's own value, by reason (`node_cap`, `time_budget`, `no_optimizer_result`). `{}` means every value is the optimizer's, and therefore that nothing in the run is machine-dependent. **The substituted bound is not tight**: measured at up to +0.0588 above the optimizer's value on the star shape at the cap boundary (ADR 0026's table, `scripts/ged_cost_benchmark.py`). Since `ged_macro` is lower-is-better, a substituted item is penalised, so an arm with more fallback items carries a small systematic worse-`ged_macro` bias — read these counts beside any `ged_macro` comparison |
...
- `max_nodes_for_optimizer` (16): above it the optimizer is not called and the value is the
  ...
  **the only guard that actually bounds the cost**, because it is checked before any search
  ...
  timing, because the cost rises with three things and the gold's depth is one of them: the
  prediction's step labels tying with **each other**, the prediction carrying `#k` references
  at all, and the gold's depth. Not the edge count (the densest graph measured, 435 edges, is
  among the fastest), and **not** resemblance to the gold's own text — a label sharing no word
  with the gold costs the same as a label copied from it, once ties and references are held
  fixed. The worst case measured at the cap is ~1.0 s
  against a 4-hop gold, against ~18.6 s at 30 nodes. The table is in ADR 0026, generated by
  `scripts/ged_cost_benchmark.py` (`configs/ged_cost_benchmark.json`), which constructs every
  shape explicitly and prints the commit it was measured at — re-run it rather than trusting
  the prose.
