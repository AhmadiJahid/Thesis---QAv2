...
**Cross-encoder claim verification (2026-08-20):** the exact-match half of the sentence above was recomputed for all 15 pairs and **stands as written**. McNemar and the paired t-test are emitted by committed code (`scripts/musique_decompositions_evaluator.py --compare --v1-per-item --v1-alignment position`, b0a9ce8, seed 42, α = 0.05, n = 750 per pair, `question` and `gold_steps` verified equal on 750/750 rows of every pair): exact match is significant in **0/15** pairs under each, smallest uncorrected p = 0.324 (McNemar) and 0.2501 (t), signs 7 positive / 8 negative. The three remaining quantities here are **not emitted by committed code** — they were recomputed by a session-local harness that reproduced the committed note's JSON bit-identically (480 field comparisons, 0 mismatches), and each is one of the harness-only families ADR 0020 lists: the EM bootstrap CIs (**family 1**) exclude zero in 0/15, nearest endpoint 0.0040 from it; the Holm adjustments (**family 4**, arithmetic over the emitted p-values) leave nothing in either 15-test family, smallest adjusted p = 1.0000; and the EM minimum detectable effect at n = 750, 0.0140–0.0227, is the committed note's **family 3** power block. No EM cell is near the threshold, unlike the pool-size claim in the paragraph below. The ROUGE-L half reproduces too: 15/15 positive, 8 significant uncorrected, the same 5 surviving Holm (family 4 again, adjusted p 0.0030–0.0302). One caveat stays attached: 0/15 is **not** evidence of no effect — the detectable effect above exceeds the largest observed gap (0.0093, itself a point estimate whose CI straddles zero), so this family cannot resolve an effect as small as the largest gap observed.
