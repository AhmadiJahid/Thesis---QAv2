...
**[Verified 2026-08-20 from committed code (issue #6 follow-up).** The exact-match half of the sentence above was recomputed for all 15 pairs with the committed battery (`scripts/musique_decompositions_evaluator.py --compare --v1-per-item --v1-alignment position`, b0a9ce8, seed 42, α = 0.05, n = 750 per pair, positional alignment with `question` and `gold_steps` verified equal on 750/750 rows of every pair). **It stands as written:** exact match is significant in **0/15** pairs under each of the three families — McNemar (smallest uncorrected p = 0.324), the paired t-test (smallest uncorrected p = 0.2501) and the bootstrap CI (every interval straddles 0, nearest endpoint 0.0040 from it) — signs 7 positive / 8 negative, and Holm within either 15-test family leaves nothing (smallest adjusted p = 1.0000). Unlike the pool-size claim in the paragraph below, no EM cell is near the threshold. The ROUGE-L half reproduces too: 15/15 positive, 8 significant uncorrected, the same 5 surviving Holm (adjusted p 0.0030–0.0302). Two caveats stay attached: 0/15 is **not** evidence of no effect — at n = 750 the EM minimum detectable effect is 0.0140–0.0227 while the largest observed gap is 0.0093, so this family cannot resolve an effect of the size present; and the EM bootstrap CIs are still the harness-only family 1 of ADR 0020 (the committed code bootstraps four other statistics), reproduced here bit-identically against the note's committed JSON rather than derived from committed code.**]**
